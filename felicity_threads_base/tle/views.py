...
def index(request):
    ...
    if request.user.is_authenticated() is False:
        return render(request, 'tle/index.html', {'notifs': notifs, 'user_nick':None})
...
def problems(request):
    ...
    return render(request, 'tle/problems.html', {'problem_data':problem_data, 'user_nick':profile.user_nick})
...
def accepted(request, uid):
    ...
    return render(request, 'tle/accepted.html', {'problem_data':problem_data, 'user_nick':profile_curr.user_nick, 'look_nick': profile.user_nick})
...
def question(request, level, id):
    ...
    if len(question_data):
        ...
        return render(request, 'tle/question.html', {'question_data':question_details, 'user_nick':user_nick, 'question_comments':question_comments})
    else:
...
def comment_submit(request, level, id):
    ...
    return HttpResponseRedirect('/contest/tle/question/'+level+'/'+id)
...
def scoreboard(request):
    ...
    if request.user.is_authenticated():
        profile = User.objects.filter(user_username = request.user.username)[0]
        user_nick = profile.user_nick
    return render(request, 'tle/scoreboard.html',  {'user_nick':user_nick})
